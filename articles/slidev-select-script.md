...
- 記載したコードの実行はWindows環境でのみ確認しています
...
基本的にはslidevのCLIにて任意のスライドを選択するコマンドが用意されていたのでそれを利用しています。  
